...
namespace ConContactBook
{
    class Program
    {
        ...
        {
    using (SQLiteConnection connection = new SQLiteConnection("Data Source=testDB.db"))
    {
        connection.Open();
        var tableExists = false;
        using (SQLiteCommand command = new SQLiteCommand("SELECT name FROM sqlite_master WHERE type='table' AND name='contact';", connection))
        {
        ...
        if (!tableExists)
        {
            string createTableSql = "CREATE TABLE contact (name TEXT NOT NULL, phone_number TEXT NOT NULL, email TEXT NOT NULL);";
